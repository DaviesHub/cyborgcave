...
plt.ylabel('Sigma')
plt.title('CNN-UV Model Predictions vs Actual Data')
plt.legend()
plt.grid(True)
plt.show()
...
plt.title('CNN-UV Validation Predictions vs Actuals')
plt.xlabel('Batch Number')
plt.ylabel('Sigma')
plt.grid(True)
plt.legend()
...
plt.title('CNN-UV Model Forecast vs Actual Data')
plt.xlabel('Batch Number')
plt.ylabel('Sigma')
plt.grid(True)
plt.legend()
...
# # Write the Python array to a CSV file
# with open(csv_file_path, 'w', newline='') as csvfile:
#     writer = csv.writer(csvfile)
#     writer.writerow(["Prediction"])  # Write header
#     writer.writerows(test_predictions.reshape(-1, 1))